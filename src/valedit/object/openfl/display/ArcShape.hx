...
class ArcShape extends BaseShape 
{
	...
	override function refreshGraphics():Void 
	{
		...
		for (i in 0...segs)
		{
			...
			bx = ax + Math.cos(angle) * this._radiusX;
			by = ay + Math.sin(angle) * this._radiusY;
			cx = ax + Math.cos(angleMid) * (this._radiusX / Math.cos(theta / 2));
			cy = ay + Math.sin(angleMid) * (this._radiusY / Math.cos(theta / 2));
